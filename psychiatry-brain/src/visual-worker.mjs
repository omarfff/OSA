...
const daySeed = Math.floor(Date.now() / 86_400_000) * count;

async function generateOne(variant) {
  const response = await fetch(`${base}/visuals/generate`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ auto: true, language: 'bilingual', variant }),
...
for (let i = 0; i < count; i += 1) {
  try {
    const result = await generateOne(daySeed + i);
    results.push({
      ok: true,
      accepted: result.accepted,
      id: result.metadata?.id || null,
      target: result.metadata?.topic || result.target?.topic || null,
      qaScore: result.metadata?.qaScore || result.score || null,
      formattingFallbackUsed: result.metadata?.formattingFallbackUsed ?? null
    });
  } catch (err) {
